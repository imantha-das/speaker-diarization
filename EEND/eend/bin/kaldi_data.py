...
class KaldiData:
    def __init__(self, data_dir):
        ...
        self.segments = load_segments_rechash(
                os.path.abspath(os.path.join(self.data_dir, 'segments')))
        self.utt2spk = load_utt2spk(
                os.path.abspath(os.path.join(self.data_dir, 'utt2spk')))
        self.wavs = load_wav_scp(
                os.path.abspath(os.path.join(self.data_dir, 'wav.scp')))
        self.reco2dur = load_reco2dur(
                os.path.abspath(os.path.join(self.data_dir, 'reco2dur')))
        self.spk2utt = load_spk2utt(
                os.path.abspath(os.path.join(self.data_dir, 'spk2utt')))

    def load_wav(self, recid, start=0, end=None):
        fn = self.wavs[recid]
        #print("wav_fn", fn)
        data, rate = load_wav(fn, start, end)
        return data, rate
